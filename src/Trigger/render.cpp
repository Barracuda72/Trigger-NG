...
void MainApp::renderVehicle(PVehicle* vehic, PGhost::GhostData* ghostdata, const glm::mat4& mv, const glm::mat4& p)
{
  ...
  for (unsigned int i = 0; i < vehic->part.size(); ++i) {
    if (vehic->type->part[i].model) {
      ...
      mat44f vorim = is_ghost ? ghostdata->ori.getMatrix().transpose() :
                     vehic->part[i].ref_world.getInverseOrientationMatrix();
    ...
    if (vehic->type->wheelmodel) {
      for (unsigned int j = 0; j < vehic->type->part[i].wheel.size(); j++) {
        ...
        mat44f worim = is_ghost ? ghostdata->wheel[j].ori.getMatrix().transpose() :
                       vehic->part[i].wheel[j].ref_world.getInverseOrientationMatrix();
...
void MainApp::renderStateChoose(float eyetranslation)
{
  ...
  getSSRender().drawText(vtype->proper_name.substr(0, 9), cl_header, PTEXT_HZA_LEFT | PTEXT_VTA_CENTER, q * scale_big,
                         o2);

  q = glm::translate(glm::mat4(1.0f), glm::vec3(100.0f, 200.0f, 0.0f));
  getSSRender().drawText(vtype->proper_class.substr(0, 8), cl_strong, PTEXT_HZA_LEFT | PTEXT_VTA_CENTER, q * scale_small,
                         o2);
  ...
  getSSRender().drawText(std::to_string(static_cast<int>(vtype->mass)), cl_strong, PTEXT_HZA_LEFT | PTEXT_VTA_CENTER,
                         q * scale_big, o2);
  ...
  else if (lss.state == AM_TOP_LVL_PREP)
  ...
  if (vtype->getLocked()) {
    ...
    if (unlockevent != "") {
      ...
      getSSRender().drawText("Complete event " + unlockevent + " to unlock", cl_marked, PTEXT_HZA_CENTER | PTEXT_VTA_CENTER,
                             q * scale_small, o2);
    }
  }
...
void MainApp::renderRpmDial(float rpm, const glm::mat4& p)
{
  ...
  glm::mat4 t = glm::translate(glm::mat4(1.0f), glm::vec3(hratio * (1.f - (2.5f / 50.f)) - 0.3f,
                               -vratio * (43.5f / 50.f) + 0.22f, 0.0f));
...
void MainApp::renderMapMarker(const glm::vec2& vpos, float angle, const glm::vec4& col, float sc, const glm::mat4& mv,
                              const glm::mat4& p)
{
  glm::mat4 t(1.0f);
...
void MainApp::renderStateGame(float eyetranslation)
{
  ...
  if (showui) {
    ...
    */
    ...
    {
      glm::mat4 t = glm::translate(glm::mat4(1.0f), glm::vec3(-hratio + hratio * (2.5f / 50.f),
                                   vratio - vratio * (5.5f / 50.f), 0.0f));
    ...
    // show Next/Total checkpoints
    {
      ...
      // checkpoint position
      glm::mat4 k = glm::translate(glm::mat4(1.0f), glm::vec3(hratio - hratio * (2.5f / 50.f),
                                   vratio - vratio * (5.5f / 50.f), 0.0f));
    ...
    if (game->number_of_laps > 1) {
      ...
      glm::mat4 k = glm::translate(glm::mat4(1.0f), glm::vec3(hratio - hratio * (2.5f / 50.f),
                                   vratio - vratio * (5.5f / 50.f) - 0.20f, 0.0f));
      k = glm::scale(k, glm::vec3(0.125f, 0.125f, 1.0f));
    ...
    if (!game->codrivercheckpt.empty() && vehic->nextcdcp != 0) {
      ...
      getSSRender().drawText(game->codrivercheckpt[vehic->nextcdcp - 1].notes, cl_codrv, PTEXT_HZA_CENTER | PTEXT_VTA_CENTER,
                             k);
    }
#endif
    ...
    // show current gear and speed
    {
      ...
      glm::mat4 k = glm::translate(glm::mat4(1.0f), glm::vec3(hratio * (1.f - (2.5f / 50.f)) - 0.3f,
                                   -vratio * (43.5f / 50.f) + 0.21f, 0.0f));
