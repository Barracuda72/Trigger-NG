...
void MainApp::tickStateGame(float delta)
{
  ...
  if (ctrl.map[PConfig::ActionLeft].type == PConfig::UserControl::TypeJoyAxis ||
    ...
    vehic->ctrl.turn.z -= ctrl.map[PConfig::ActionLeft].value;
    vehic->ctrl.turn.z += ctrl.map[PConfig::ActionRight].value;
    ...
    if (ctrl.map[PConfig::ActionLeft].value > 0.0f) {
      if (turnaccel > -0.0f) turnaccel = -0.0f;
      turnaccel -= 8.0f * delta;
      vehic->ctrl.turn.z += turnaccel * delta;
    } else if (ctrl.map[PConfig::ActionRight].value > 0.0f) {
      if (turnaccel < 0.0f) turnaccel = 0.0f;
      turnaccel += 8.0f * delta;
      vehic->ctrl.turn.z += turnaccel * delta;
    } else {
  ...
  if (ctrl.map[PConfig::ActionForward].value > 0.0f) {
    if (vehic->wheel_angvel > -10.0f)
      throttletarget = ctrl.map[PConfig::ActionForward].value;
    else
      braketarget = ctrl.map[PConfig::ActionForward].value;
  }
  if (ctrl.map[PConfig::ActionBack].value > 0.0f) {
    if (vehic->wheel_angvel < 10.0f)
      throttletarget = -ctrl.map[PConfig::ActionBack].value;
    else
      braketarget = ctrl.map[PConfig::ActionBack].value;
  }
