...
glm::mat4 PApp::stereoFrustum(float xmin, float xmax, float ymin, float ymax, float znear, float zfar, float zzps,
                              float eye)
{
    ...
    while ( SDL_PollEvent(&event) ) {
      switch (event.type) {
        ...
        case SDL_JOYAXISMOTION:
          ...
          axis_down = joyAxisEvent(event.jaxis.which, event.jaxis.axis, sdl_joy[event.jaxis.which].axis[event.jaxis.axis],
                                   axis_down);
          break;
...
void PApp::drawModel(PModel &model, const Light& light, const Material& material, bool is_ghost, const glm::mat4& mv,
                     const glm::mat4& p)
{
